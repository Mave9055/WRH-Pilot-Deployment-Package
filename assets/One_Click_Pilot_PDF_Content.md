# CAPITOL CONTRACTS LLC — ARKANSAS SDVOSB
## **WRH Pilot Deployment Package: Pilot Site A — Stepping Stones Recovery**
### **Official 30-Session Pilot Proposal & Program Overview**
**Date:** April 2026  
**Founder:** [Your Name], Service-Disabled Veteran Owner  
**Location:** Clarksville, Arkansas  
**Contact:** [Your Email] | [Your Phone]

---

## **1. EXECUTIVE SUMMARY**
**Capitol Contracts LLC** presents the **What Really Happened (WRH)** curriculum — a 30-session psychoeducational program designed to interrupt trauma-driven behavioral loops and restore executive function. 

As a **Service-Disabled Veteran-Owned Small Business (SDVOSB)**, our mission is to deliver systems-level tools that address the root causes of isolation, avoidance, and suicide risk. WRH is built on a **non-exposure, mechanism-first model**, allowing participants to understand their nervous system's survival code without being forced to recount personal trauma.

---

## **2. PILOT SITE A: STEPPING STONES RECOVERY OF ARKANSAS**
**Location:** 1561 County Road 2305, Hartman, AR  
**Status:** Zero-Cost Pilot (2026 Launch)

### **Zero-Cost Framing**
This pilot is a **zero-cost mission-aligned investment** for the 2026 launch phase. Capitol Contracts LLC provides the full curriculum, facilitator training, and digital assets at no cost in exchange for high-fidelity delivery and anonymized outcome data.

### **Residential Integration**
- **Scheduling:** Integrated into existing residential routines (SUD/Recovery).
- **Peer Pods:** Small subgroups for tool-usage check-ins and peer support.
- **Voluntary Participation:** Choice-based engagement to maintain safety.

---

## **3. THE WRH CURRICULUM: 3-ARC STRUCTURE**
The 30-session journey is divided into three logical arcs:

### **Arc 1: The Machine (Sessions 00–10)**
*How the system gets built into you.*  
Focus: The Hum, The Mask, The Crash, and The Loop.

### **Arc 2: The Drivers (Sessions 11–20)**
*What keeps the engine running.*  
Focus: Triggers vs. Activation, The Threshold, and Time Collapse.

### **Arc 3: The Interrupt & Restore (Sessions 21–30)**
*How you rewrite the code.*  
Focus: The Pattern Veto, Executive Function Restore, and The Code Rewrite.

---

## **4. RETENTION & SAFETY PROTOCOLS**
### **Beating the "Learn 20, Forget 80" Pattern**
- **20:80 Application Drills:** 80% of session time is dedicated to practical application.
- **Spaced Boosters:** Automated prompts at 1, 3, 7, and 30 days post-session.
- **One-Tool Commitment:** Practical focus on one specific tool between sessions.

### **Safety & Risk Management**
- **Yellow/Red Light Protocol:** Standardized room regulation and opt-out options.
- **Warm-Handoff Protocol:** Immediate connection to the **Veterans Crisis Line (988, press 1)** or on-site clinical staff.
- **Non-Clinical Boundary:** WRH is psychoeducational, not therapeutic.

---

## **5. SUCCESS CRITERIA & NEXT STEPS**
**Success Indicators:**
- 80%+ participant retention across the 30-session cohort.
- Self-reported application of at least three WRH tools in daily life.
- Actionable data for scaling to future federal and state sites.

**Next Steps for Stepping Stones:**
1. **Review:** Leadership review of this proposal and the live deployment site.
2. **Plan:** Joint call to finalize the cohort start date and logistics.
3. **Launch:** Pilot commencement with weekly fidelity check-ins.

---
### **CAPABILITY STATEMENT (SUMMARY)**
- **UEI:** [Your UEI] | **CAGE:** [Your CAGE]
- **NAICS:** 611710 (Educational Support Services)
- **Status:** SDVOSB (Service-Disabled Veteran-Owned Small Business)

---
*Capitol Contracts LLC | WRH Pilot Deployment | April 2026*
